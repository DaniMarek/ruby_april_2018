...
def getAvg arr
	puts (arr.reduce(:+) / arr.length.to_f).to_s
end
...
x = [1, 5, 10, -2]
p x.each_index { |i| x[i]=0 if x[i] < 0 }
...
x = [1, 5, 10, -2]
p hash = { max: x.max, min: x.min, avg: x.reduce(:+) / x.length.to_f }
...
x = [-1, -3, 2]
p x.each_index { |i| x[i] = 'Dojo' if x[i] < 0 }